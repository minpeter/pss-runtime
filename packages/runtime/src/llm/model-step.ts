...
export async function generateModelStepResult({
	...
	const materializedTools = contextTokenMeter || contextGate
		? await materializeModelPromptTools(prepared.tools)
		: {};
	const visibleTools = materializedTools.promptTools;
	const promptMeasurement = contextTokenMeter || contextGate
		? (
	...
	const handle = createModelStepStream({
		...
		onAttemptEnd: (origin) => {
			const event = attemptTracker.succeed(origin);
			if (event) {
				onStreamEvent?.(event);
			}
		},
		onAttemptError: (error) => {
			attemptTracker.observeFailure(error);
		},
	...
	try {
		for await (const part of handle.parts) {
			...
			if (event) {
				...
				const usageSnapshot = delta === undefined
					? undefined
...
export function snapshotModelHistory(
	...
	for (let index = 0; index < lengthDescriptor.value; index += 1) {
		...
		try {
			...
			throw new TypeError(
				"history contains an invalid message descriptor.",
			);
		}
