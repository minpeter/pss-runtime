...
describe("ThreadHandle public API", () => {
  ...
  it("exposes dispose instead of kill", () => {
    ...
    expect(getProperty(thread, "dispose")).toBeTypeOf("function");
    expect(getProperty(thread, "kill")).toBeUndefined();
  });

  it("drives thread runs with MockLanguageModelV4 and records model calls", async () => {
