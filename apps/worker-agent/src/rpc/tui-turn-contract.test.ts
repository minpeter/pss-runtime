...
import { TuiTurnOutputSchema } from "../tui/tui-contract";
...
const BOUNDED_UPSTREAM_MESSAGE = "agent durable object unavailable";
...
describe("tui.turn tRPC contract", () => {
  beforeEach(() => {
    durableObjectMock.requests.length = 0;
    durableObjectMock.outcomes.length = 0;
  });

  it("returns the tRPC success envelope with a schema-valid delivery", async () => {
    ...
    const response = await tuiTurn(VALID_INPUT);

    expect(response.status).toBe(200);
    const envelope = (await response.json()) as {
      readonly result?: { readonly data?: Record<string, unknown> };
    };
    const parsed = TuiTurnOutputSchema.safeParse(envelope.result?.data);
    expect(parsed.success).toBe(true);
    if (!parsed.success) {
      throw new Error("expected a schema-valid delivery envelope");
    }
  ...
  it.each([
    ["a steered delivery without messages", { delivered: true, mode: "steer" }],
    [
      "a missing-send-message delivery failure",
      { delivered: false, error: "missing_send_message", mode: "send" },
    ],
  ])("returns the documented union member for %s", async (_label, body) => {
    durableObjectMock.outcomes.push({
      kind: "respond",
  ...
  it("trims turn fields and omits a blank sessionScopeKey before the hop", async () => {
    const response = await tuiTurn({
      channel: { id: " local ", kind: "tui" },
      sessionScopeKey: "   ",
      text: " hello ",
    });
    ...
    expect(new URL(request.request.url).pathname).toBe("/turn");
  ...
  it("forwards a trimmed sessionScopeKey", async () => {
    const response = await tuiTurn({
      channel: { id: "local", kind: "tui" },
  ...
  it("rejects a telegram channel with the documented tui-only message", async () => {
    const response = await tuiTurn({
    ...
    expect(body).toContain("text and tui channel required");
  ...
  it.each([
    ...
    async (_label, input) => {
      const response = await tuiTurn(input);
  ...
  it("rejects a GET at the adapter before handler logic", async () => {
    const response = await probeMethod("/trpc/tui.turn", "GET");
...
describe("tui.turn upstream failure mapping", () => {
  beforeEach(() => {
    durableObjectMock.requests.length = 0;
    durableObjectMock.outcomes.length = 0;
  });
  ...
  it("maps an unreachable Durable Object to a bounded BAD_GATEWAY", async () => {
    ...
    expect(body).toContain(BOUNDED_UPSTREAM_MESSAGE);
    expectBoundedErrorBody(body);
  });
  ...
  it("maps a schema-invalid Durable Object payload to a generic 500", async () => {
    durableObjectMock.outcomes.push({
      kind: "respond",
      response: Response.json({ delivered: "maybe", reason: "refused" }),
    });
...
function tuiTurn(input: unknown): Promise<Response> {
  return probeMethod("/trpc/tui.turn", "POST", input);
}
